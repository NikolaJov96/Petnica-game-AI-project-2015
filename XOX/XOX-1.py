...
class QLearner:
    ...
    def makeMove(self, scr, repLast, used):
        ...
        if self.gamma == 2:
            ...
            for i in range(1,9):
                if i in actions:
                    if self.vMat[num1][i] > self.vMat[num1][self.preA]:
                        self.preA = i
        elif random.random() < self.gamma:
            # Chosing action by using weighted random
            su = 0
            usu = 0
            for a in actions: usu = usu + self.vMat[num1][a]
            rnd = random.randint(0, int(usu))
            for a in actions:
                su = su + self.vMat[num1][a]
                if rnd <= su:
                    self.preA = a
                    break
        else:
...
UseRandom = False
vMat = [[1024.0 for i in range(9)] for j in range(20000)]
...
for its in range (iters):
    ...
    qLearner1 = QLearner(vMat, gamma, aTT1)
    qLearner2 = QLearner(vMat, gamma, aTT2)
